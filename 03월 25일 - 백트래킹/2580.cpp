...
bool fillSudoku(int idx) { // 스토쿠 채우는 함수
    if (idx == MAX * MAX) { // 스도쿠 모두 채움 (기저 조건)
        return true; // true return
    }

    int row = idx / MAX; // 행은 idx / 9
    int col = idx % MAX; // 열은 idx % 9

    if (sudoku[row][col]) { // 이미 숫자가 채워진 칸이라면 다음 칸으로 넘어감
        return fillSudoku(idx + 1); // 다음칸으로
    }

    for (int i = 1; i <= MAX; i++) { // 1~9까지 넣어보기
        if (!check_row[row][i] && !check_col[col][i] && !check_3x3[(row / 3) * 3 + col / 3][i]) {
            check_row[row][i] = true; // (idx / 9) 행에 i라는 숫자가 존재함
            check_col[col][i] = true; // (idx % 9) 열에 i라는 숫자가 존재함
            check_3x3[(row / 3) * 3 + col / 3][i] = true; // 3x3 사각형에 존재함
            sudoku[row][col] = i; // 스도쿠 정보에 1 대입
            if (fillSudoku(idx + 1)) { // fillSudoku(inx+1)이 true 이면
                return true; // 생각해보기 : 이 부분이 없으면 어떻게 될까요?
            }
            check_row[row][i] = false; // (idx / 9) 행에 i라는 숫자가 존재 안 함
            check_col[col][i] = false; // (idx / 9) 행에 i라는 숫자가 존재 안 함
            check_3x3[(row / 3) * 3 + col / 3][i] = false; // 3x3 사각형에 존재 안 함
            sudoku[row][col] = 0; // 스도쿠 정보에 0 대입
        }
    }
    ...
    for (int i = 0; i < MAX; i++) {
        for (int j = 0; j < MAX; j++) {
            ...
            if (sudoku[i][j]) { // 스도쿠 상태 보고, check배열들 초기화
                check_row[i][sudoku[i][j]] = true; // i행에 sudoku[i][j] 존재함
                check_col[j][sudoku[i][j]] = true; // j열에 sudoku[i][j] 존재함
                check_3x3[(i / 3) * 3 + j / 3][sudoku[i][j]] = true; // 3x3 사각형에 suduko[i][j] 존재함
            }
        }
    }
