...
using namespace std; // std:: 사용하지 않기 위해서
...
int chessChange(int row, int col, vector<vector<char>> &board) {
    ...
    for (int i = 0; i < 8; i++) { // 행 변화값
        for (int j = 0; j < 8; j++) { // 열 변화값
            if ((i + j) % 2 == 0 && board[row + i][col + j] != 'B') { // (i + j) % 2 == 0이면서 문자가 B가 아니면
                b_cnt++; // b_cnt 증가
            } else if ((i + j) % 2 && board[row + i][col + j] != 'W') { // (i + j) % 2 == 1이면서 문자가 W가 아니면
                b_cnt++; // b_cnt 증가
            }
    ...
    // 최솟값 리턴
    if (b_cnt > SIZE / 2) { // b_cnt가 32보다 크면
        return SIZE - b_cnt; // 32 - b_cnt
    }
    return b_cnt; // b_cnt return
}
...
int main() {
    ...
    // 연산
    for (int i = 0; i <= n - 8; i++) { // n-8까지 반복
        for (int j = 0; j <= m - 8; j++) { // m-8까지 반복
