//
// Created by haewonny on 2022-03-23.
//
...
stack<int> calcPlus(string &a, string &b) {
    ...
    while (p1 >= 0) { // a에서 남은 숫자 반영
        ...
        num += carry; // num에 carry 더하기
        carry = num / 10; // 자리수 더한 값이 10을 넘으면 올림변수 체크
        st.push(num % 10); // 올림값 제외한 값만 스택에 저장
    }
...
int main() {
    ...
    st = calcPlus(a, b); // 함수 사용

    // 출력
    while (!st.empty()) { // st가 비어있지 않은 동안
        cout << st.top();
        st.pop(); // 삭제
    }
   cout << '\n';
